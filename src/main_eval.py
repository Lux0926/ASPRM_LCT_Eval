...
if __name__ == "__main__":
    ...
    for data in data_list:
        ...
        # response = re.search(r'### Response:\s*(.*)', data["answer"], re.DOTALL)
        # answer = response.group(1)
        # print(extracted_content)
        # import time
        # time.sleep(100)
        answer = data["solution_min"]
        generations = [{
    ...
    min_score_final = min_score/len(data_list)
    mean_score_final = mean_score/len(data_list)
    max_score_final = max_score/len(data_list)
    with open("result_all.jsonl", "a", encoding="utf-8") as ff:
        ff.write(input_file+":"+"mean_score:"+str(mean_score_final)+","+"max_score:"+str(max_score_final)+","+"min_score:"+str(min_score_final)+",")
        ff.write("\n")
    print(len(data_list))
    print(mean_score_final)
    print(max_score_final)
    print(min_score_final)
